/*--refer to Events.html--The addEventListener() node 
method is part of the DOM specification. It is 
supported by most modern browsers, but not by older 
versions of Internet Explorer. In IE8 and earlier, you
must use IE's proprietary attachEvent() method, which
essentially works the same way. The only difference is
 that in IE, the attachEvent() method will repeatedly 
attach the same event to an event handler when called 
multiple times using the same parameters. Our 
observeEvent() function above attaches events to nodes
